...
import { getSplunkClient } from './splunk-client.js';
...
export interface HedgeConfig {
  ...
  // Optional: route the hedge attempt through a different provider client.
  // 'tf' (default) keeps existing behavior; 'splunk' uses getSplunkClient()
  // and the hedgeModel must be one of getSplunkModels().
  hedgeVia?: 'tf' | 'splunk';
}
...
export async function hedgedChatCompletion(
  ...
  let hedgeStart = 0;
  const hedgeVia: 'tf' | 'splunk' = config.hedgeVia ?? 'tf';
  const hedgeClient: OpenAI = hedgeVia === 'splunk' ? getSplunkClient() : tf;

  const hedgeTimer = setTimeout(() => {
    hedgeFired = true;
    hedgeStart = Date.now();
    hedgeClient.chat.completions
