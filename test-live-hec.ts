...
const events = [
  {
    sourcetype: 'aegis:chaos' as const,
    event: {
      drill_id: 'd-001',
      target: 'anthropic',
      mode: 'shadow',
  ...
  },
  {
    ...
    event: {
      ...
      verdict: 'PASS',
      receipt_sig: 'ed25519:8a4b',
    },
  },
  {
    sourcetype: 'aegis:mcp-failover' as const,
    event: {
      request_id: 'req-9f1',
...
for (const evt of events) {
  ...
  console.log(
    `[${evt.sourcetype}] attempted=${r.attempted} ok=${r.ok} status=${r.status ?? '-'} error=${r.error_class ?? '-'}`,
  );
}
